...
class ObjectManager<T extends Model> {
  ...
  Future refresh() async {
    var model = await Ajax.request_decode_from_model(
        this.updated, 'GET', this.object_url,
        data: {});
    this.original = model.clone() as T;
    this.updated = model.clone() as T;
  }
  ...
  Future save() async {
    ...
    this.updated.props.forEach((key, val) => {
          if (val != this.original.props[key]) {to_send[key] = val}
        });

    var model = await Ajax.request_decode_from_model(
        this.updated, 'PATCH', this.object_url,
        data: to_send);
